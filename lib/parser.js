...
//  link: Map(slug, Set())
let stopword;
export function parseHTML(html, DOMselector, domain, slug, indexFile = 'index.html', stopWordLen = 7) {
  ...
  stopword = stopword || stopWords( stopWordLen );
  ...
  // extract words
  res.word = {
    title: wordSet( res.title ),
    description: wordSet( res.description ),
    content: new Set(),
    emphasis: new Set(),
    alt: new Set(),
    h2: new Set(),
  ...
  DOMselector.forEach(d => {
    ...
    // all content
    res.word.content = res.word.content.union( wordSet( node.structuredText ) );
    ...
    res.word.alt = res.word.alt.union(
      wordSet( node.querySelectorAll('img').map(i => i.getAttribute('alt')).join(' ') )
    );
    ...
    for (let h = 2; h <= 6; h++) {
      ...
      res.word[head] = res.word[head].union(
        wordSet( node.querySelectorAll(head).map(t => t.structuredText).join(' ') )
      );
    }
  ...
  function wordSet(str) {
    ...
    return new Set(
      str.toLowerCase()
        ...
        .trim()
        .split(/\s+/g)
        .map(w => stem(w).slice(0, stopWordLen))
        .filter(w => w && !stopword.has(w))
    );
